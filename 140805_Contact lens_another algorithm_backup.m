    ...
    for p=1:NNN/Jump
        ...
        elseif (q==1)
            ...
            if Jump==1
                ...
                %[max_value max_index]=max(Image_Polar((p*Jump),(Max_line_index(((p-1)*Jump),q)-tolerance):(Max_line_index(((p-1)*Jump),q)+tolerance)));
                [min_value min_index]=findpeaks(max(max(Image_Polar))-Image_Polar(p*Jump,(Max_line_index(p*Jump,q-1)):end),'NPEAKS',1);
                [man_value max_index]=findpeaks(Image_Polar(p*Jump,(Max_line_index(p*Jump,q-1)+next_min_index):end),'NPEAKS',1);
                Start_Index=min_index+max_index+Max_line_index(p*Jump,q-1)-1;
