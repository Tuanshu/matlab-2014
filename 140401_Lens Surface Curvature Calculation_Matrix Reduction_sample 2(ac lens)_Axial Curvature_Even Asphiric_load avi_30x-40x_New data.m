...
%% period 80s, amp 1Vpp, offset 5V, DC20%
%% Height generation
Averaging_Factor=1;
Binning_Factor=1;
...
LPF=90;   %pixel
%% Height generation
cd('D:\Users\TuanShu\140401');
MOVIE=aviread('140401_30x-40x_realigned.avi');
%%
...
Z_ref=dlmread('Z_Ref.txt');
X_Shift=0;
Y_Shift=0;
Z_sum=circshift(Z_ref,[X_Shift Y_Shift])-Z_mod;

imagesc(Z_sum,'xdata',Lateral_Spacing:Lateral_Spacing:(size(Z,2)*Lateral_Spacing),'ydata',Lateral_Spacing:Lateral_Spacing:(size(Z,1)*Lateral_Spacing));
axis equal
%caxis([0 12]);
Z_sum=Z_sum-max(max(Z_sum(100:200,280:400)))/2;
xlim([Lateral_Spacing size(Z,2)*Lateral_Spacing]);
ylim([Lateral_Spacing size(Z,1)*Lateral_Spacing]);
colorbar
xlabel('(micron)');
...
Diopter(isinf(Diopter))=21;

Diopter(Diopter>400)=21;
...
imagesc(Diopter_Ave,'xdata',Lateral_Spacing*Reduction_factor:Lateral_Spacing*Reduction_factor:(size(Z,2)*Lateral_Spacing*Reduction_factor),'ydata',Lateral_Spacing*Reduction_factor:Lateral_Spacing*Reduction_factor:(size(Z,1)*Lateral_Spacing*Reduction_factor));
%plot(Axial_Radius_of_Curvature(300,:))
caxis([0 30]);
axis equal
...
Diopter_Ave_Mask=Diopter_Ave.*Mask_new;
Axial_Radius_of_Curvature_Ave_Mask=(1.3375-1)*(1000)./Diopter_Ave_Mask;


imagesc(Axial_Radius_of_Curvature_Ave_Mask,'xdata',Lateral_Spacing*Reduction_factor:Lateral_Spacing*Reduction_factor:(size(Z,2)*Lateral_Spacing*Reduction_factor),'ydata',Lateral_Spacing*Reduction_factor:Lateral_Spacing*Reduction_factor:(size(Z,1)*Lateral_Spacing*Reduction_factor));
%plot(Axial_Radius_of_Curvature(300,:))
caxis([0 30]);
axis equal
