...
cd('D:\Users\TuanShu\140128\lens_2\');

Stage_speed=20;  %micron/sec
...
image_index=1:2500;
ROI=[1 512;1 271];      %up, down, left, right
Image_Stack(length(image_index),1:(ROI(1,2)-ROI(1,1)+1),1:(ROI(2,2)-ROI(2,1)+1))=0;
Low_Pass=350;   %pixel
High_Pass=550;
Position=Frame_Axial_Spacing*image_index;
for p=1:length(image_index)
    Image=dlmread(sprintf('140128_lens_2_%i',image_index(p)));    
...
%plot(real(FFT_Stack(:,150,100)));
plot(Position,real(Image_Stack(:,150,100)));
