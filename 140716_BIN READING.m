tic
clear all

%% 8-bit binary
cd('C:\Personal\Yao sheng\0609_ALLstray\0609_ALLstray1');

mkdir('divide')
N_frame=1452;

num_of_division=10;

for slice_mun=1:N_frame
fin=fopen(['300umD263_',sprintf('%d',slice_mun),'.']);
A=fread(fin,[512 2710],'uint8','b');

for i=1:num_of_division
    slice=A(:,(271*(i-1)+1):271*i);
%     dlmwrite([cd,'\divide\',sprintf('%d',(slice_mun-1)*10+i),'.txt'],slice);
    imwrite(slice/255,[cd,'\divide\',sprintf('%d',(slice_mun-1)*10+i),'.bmp']);
end

fclose('all');
end

toc 



 
%% 16-bit binary
tic

cd('C:\Personal\Yao sheng\0630_dZ135um_BCC_ADC');

mkdir('divide')
N_frame=9759;
size_x=148;
size_y=150;
num_of_division=10;

for slice_mun=1:N_frame
fin=fopen(['dZ135um_',sprintf('%d',slice_mun),'.']);
A=fread(fin,[size_x,size_y*num_of_division],'*uint16','b');
imagesc(A);
colormap gray

for i=1:num_of_division
    slice=A(:,(size_y*(i-1)+1):size_y*i);
%     dlmwrite([cd,'\divide\',sprintf('%d',(slice_mun-1)*10+i),'.txt'],slice);
    imwrite(slice,[cd,'\divide\',sprintf('%d',(slice_mun-1)*num_of_division+i),'.png']);
end

fclose('all');
% imagesc(slice);
% colormap gray

 end

toc 

%% binary to image for FFOCT-1
clear all
tic

cd('C:\Personal\Yao sheng\(©s®x)tissue carrier data\2.carrier_PZT not move1\carrier data');

mkdir('divide')
N_frame=5400;

% fin=fopen([sprintf('%08d',(1000)),'.bin']);
%  A=(fread(fin,[648 488],'uint16','b'))';

for slice_num=1:N_frame
 fin=fopen([sprintf('%08d',(slice_num)),'.bin']);
 A=(fread(fin,[648 488],'uint16','b'))';
 imwrite(A/65535,[cd,'\divide\',sprintf('%d',slice_num),'.png'],'BitDepth',16);
 fclose('all');
end

toc 

%% binary to image for FFOCT-1  (for noncontinuous binary file)
clear all
tic

cd('C:\Personal\Yao sheng\(©s®x)tissue carrier data\2.carrier_PZT not move1\carrier data');

mkdir('divide')
N_frame=5400;
% 
% [fin,msg]=fopen([sprintf('%08d',(67)),'.bin']);
%  A=(fread(fin,[648 488],'uint16','b'))';

 k=0;
for slice_num=1:N_frame
 fin=fopen([sprintf('%08d',(slice_num)),'.bin']);
 if fin ==-1
     k=k+1;
     fclose('all');
 else
     A=(fread(fin,[648 488],'uint16','b'))';
     imwrite(A/65535,[cd,'\divide\',sprintf('%d',slice_num-k),'.png'],'BitDepth',16);
     fclose('all');
 end
end

toc 