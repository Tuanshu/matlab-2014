...
for p=1:length(image_index)
    Image_Temp=dlmread(sprintf(' 140304_aclens1_%i',image_index(p)));   
...
Z=((Surface_profile-max(max(Surface_profile))).*d_Position);
Z(isnan(Z))=0;


imagesc(Z,'xdata',Lateral_Spacing*Reduction_factor:Lateral_Spacing*Reduction_factor:(size(Z,2)*Lateral_Spacing*Reduction_factor),'ydata',Lateral_Spacing*Reduction_factor:Lateral_Spacing*Reduction_factor:(size(Z,1)*Lateral_Spacing*Reduction_factor));
axis equal
caxis([-12 0]);
