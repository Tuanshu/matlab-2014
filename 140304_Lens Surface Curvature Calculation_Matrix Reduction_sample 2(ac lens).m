...
Starting_X= 588.7973;                                                                                                                                                                                                                              
Starting_Y=859.8576;
Starting_R=2.1958e+04;
Starting_C=-2.1958e+04;
...
imagesc(CheckCurve.*Mask,'xdata',Lateral_Spacing:Lateral_Spacing:(size(CheckCurve,2)*Lateral_Spacing),'ydata',Lateral_Spacing:Lateral_Spacing:(size(CheckCurve,1)*Lateral_Spacing));
axis equal
caxis([-12 0]);

xlim([Lateral_Spacing size(CheckCurve,2)*Lateral_Spacing]);
ylim([Lateral_Spacing size(CheckCurve,1)*Lateral_Spacing]);
colorbar
xlabel('(micron)');
ylabel('(micron)');
%%                        
...
R_mean_smooth=(1./(H)/1000).*Mask_new;    %mm
R_mean_smooth(R_mean_smooth>25)=0;
...
imagesc(R_mean_smooth,'xdata',Lateral_Spacing*Reduction_factor:Lateral_Spacing*Reduction_factor:(size(Z,2)*Lateral_Spacing*Reduction_factor),'ydata',Lateral_Spacing*Reduction_factor:Lateral_Spacing*Reduction_factor:(size(Z,1)*Lateral_Spacing*Reduction_factor));
caxis([10 30]);
axis equal
...
R_mean_smooth(isnan(R_mean_smooth))=0;
%% take mean value

Mean_Curvature=1/mean(H(R_mean_smooth>0))/1000;
