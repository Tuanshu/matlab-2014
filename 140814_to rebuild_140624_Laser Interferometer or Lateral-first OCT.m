...
%% Background 1
Image_Ave=mean(Image_Stack,3);

WindowSize=50;
filter=fspecial('gaussian',WindowSize,round(WindowSize/2));
filter=filter/sum(sum(filter));
Image_Ave_Smooth=conv2(Image_Ave,filter,'same');

imagesc(Image_Ave_Smooth);

%% Background 2

for xx=1:(N_End-N_Start+1)
    if xx<9
        %Background=Image_Stack(:,:,xx+3);
    else
        %Background=Image_Stack(:,:,xx-3);
    end
    Background=Image_Ave_Smooth;
