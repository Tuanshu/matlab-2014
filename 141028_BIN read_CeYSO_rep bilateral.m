...
X_range=[9 648];
Y_range=[9 488];
Z_range=[440 1530]; %Ce:YSO glass;%[421 670] Ce:YAG cell;

cd('D:\Users\TuanShu\141022_CeYSO on BCC\bin_CeYSO_125fps_92fpp_0.2micronsecond_log_after realignment_position 3\');
...
%% 3D blurred
Smooth_window=1;
Image_Stack_Smoothed=smooth3(Image_Stack_sub,'gaussian',Smooth_window);
%%

imagesc(Image_Stack_Smoothed(:,:,N));
colormap(gray);
axis equal;
caxis([0 0.2]);
...
SIGMA=[1.5 0.3];
Iteration=10;
Image_Stack_BI=Image_Stack_Smoothed;
for q=1:Iteration
    for p=N:N%1:size(Image_Stack_sub,3)
        temp=Image_Stack_BI(:,:,p);
...
colormap(gray);
axis equal;
caxis([min(min(Image_Stack_BI(:,:,N))) max(max(Image_Stack_BI(:,:,N)))]);
%% Background sub
N=185;
Image_Stack_BND_Sub=Image_Stack_BI-min(min(min(Image_Stack_BI)));
%%
N=281;
imagesc(Image_Stack_BND_Sub(:,:,N));
colormap(gray);
axis equal;
caxis([0 max(max(Image_Stack_BND_Sub))]);
