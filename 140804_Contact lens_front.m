...
Image_test=dlmread(sprintf('upper_once_again_background__%d',NN));
imagesc(Image_test(end:-1:1,end:-1:1)');

%%
cd('D:\Users\TuanShu\140804_Contact Lens\interface 2 2\');

N1=1;  %%91
N2=2;  %%91

Image_test=dlmread(sprintf('%d',N1))-dlmread(sprintf('%d',N2));
imagesc(Image_test);
xlim([800 1300]);
ylim([800 1300]);
xlabel('Pixel Number');
...
Image=dlmread(sprintf('%d',N1))-dlmread(sprintf('%d',N2));
imagesc(Image);
colormap(gray);
axis equal
%
...
imagesc(Z_XY(:,:,N),'xdata',Lateral_Spacing:Lateral_Spacing:(size(Z_XY,2)*Lateral_Spacing),'ydata',Lateral_Spacing:Lateral_Spacing:(size(Z_XY,1)*Lateral_Spacing));
axis equal
colormap(gray);
...
Z_offset=0;
Z_total=Z_ref+Z_mod;
imagesc(Z_total,'xdata',Lateral_Spacing:Lateral_Spacing:(size(Z_total,2)*Lateral_Spacing),'ydata',Lateral_Spacing:Lateral_Spacing:(size(Z_total,1)*Lateral_Spacing));
