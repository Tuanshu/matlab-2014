...
%% the Vpp for rolloff2 is 7V
%% the Vpp for rolloff2 is 10V
...
Signal_Bscan_Carrier(1:round(N_t*ROI_ratio),1:length(array))=0;


%% Wavelength
cd('D:\Users\TuanShu\140217_sensitivity rolloff3\');
Data=importdata('D1.txt');

Background=importdata('140217_sam.txt')+importdata('140217_ref.txt')-importdata('140217_bs.txt');
...
for p=1:length(array)
    ...
    Data=importdata(sprintf('D%i.txt',array(p)));

    Spectrum=Data(:,2)-Background;
    %plot(Data(:,1),Data(:,2),Data(:,1),Background);
    ...
    Signal_Bscan_Envelope(:,p)=abs(Signal(1:size(Signal_Bscan_Envelope,1)));
    Signal_Bscan_Carrier(:,p)=real(Signal(1:size(Signal_Bscan_Envelope,1)));
    disp(p);
end

Signal_Bscan_Envelope_Ori=Signal_Bscan_Envelope;

%% cut lower part

Signal_Bscan_Envelope(1:150,:)=0;


Signal_Bscan_Envelope=Signal_Bscan_Envelope/max(max(Signal_Bscan_Envelope));

imagesc(Signal_Bscan_Envelope);

[Max_value Max_index]=max(Signal_Bscan_Envelope,[],1);

plot(Position_micron(Max_index),Max_value);


%%
Frame_Number_Considered=150:1750;
Signal_Bscan_Envelope_Patial=Signal_Bscan_Envelope(:,Frame_Number_Considered);
Signal_Bscan_Envelope_Patial=Signal_Bscan_Envelope_Patial/max(max(Signal_Bscan_Envelope_Patial));
imagesc(Signal_Bscan_Envelope_Patial);

[Max_value Max_index]=max(Signal_Bscan_Envelope_Patial,[],1);

plot(Position_micron(Max_index),Max_value);
xlabel('Optical Path Difference (micron)');
ylabel('Normalized Signal Intensity');

plot(Frame_Number_Considered,Position_micron(Max_index));

plot(Frame_Number_Considered,Max_value);
%%
...
%%
%axis equal
imagesc(Signal_Bscan_Envelope,'ydata',Position_micron);
colormap(gray);
%axis equal
xlabel('Frame Number');
...
Data=importdata(sprintf('D%i.txt',N));
...
Spectrum_New(isnan(Spectrum_New))=0;
Spectrum_New(Frequency_New<Min_Frequency)=0;
Wavelength_nm=C./Frequency_New*1E9;
plot(Wavelength_nm(abs(Wavelength_nm-600)<150),Spectrum_New(abs(Wavelength_nm-600)<150));
